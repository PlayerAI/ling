...
- [`editors/tree-sitter-ling/`](editors/tree-sitter-ling/)：可独立拆分的 Tree-sitter grammar 开发镜像，包含锁定工具链、生成 parser、41 个 grammar corpus cases、42 个全程序 compiler/Tree-sitter differential cases、84 个全语料定种子编辑和 43 个稳定 CST/node-type 映射哈希；它不决定 Ling 语义或合法性。
...
- [`editors/tree-sitter-ling/`](editors/tree-sitter-ling/): a standalone-ready Tree-sitter grammar development mirror with a locked toolchain, generated parser, 41 grammar corpus cases, 42 whole-program compiler/Tree-sitter differential cases, 84 fixed-seed whole-corpus edits, and 43 stable CST/node-type mapping hashes; it does not define Ling semantics or validity.
