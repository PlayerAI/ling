...
- Accepted `ling.manifest/1` reader 与显式工程根下的确定性 module discovery（尚未接入 CLI 或依赖解析）；
...
- an Accepted `ling.manifest/1` reader and deterministic module discovery under an explicit project root (not yet integrated with the CLI or dependency resolution);
