...
- [`editors/tree-sitter-ling/`](editors/tree-sitter-ling/)：可独立拆分的 Tree-sitter grammar 开发镜像，包含锁定工具链、生成 parser、41 个 corpus cases、29 个共享表达式 differential cases、41 个共享 pattern/type differential cases、10 个静态恢复 cases、9 个增量编辑和 64 个定种子 mutations；它不决定 Ling 语义或合法性。
...
- [`editors/tree-sitter-ling/`](editors/tree-sitter-ling/): a standalone-ready Tree-sitter grammar development mirror with a locked toolchain, generated parser, 41 corpus cases, 29 shared expression differential cases, 41 shared pattern/type differential cases, 10 static recovery cases, 9 incremental edits, and 64 fixed-seed mutations; it does not define Ling semantics or validity.
