...
| `L-IO-0002` | `IO` | `Preview` | `Error` | 无法读取或持久化工程锁文件 | Project lockfile cannot be read or persisted | 工程锁文件操作失败 | project lockfile operation failed | `io_kind:string, operation:string` | — | `0.0.1-dev` |
| `L-IO-0003` | `IO` | `Preview` | `Error` | 无法创建或提交工程脚手架 | Project scaffold cannot be created or committed | 工程脚手架文件操作失败 | project scaffold file operation failed | `io_kind:string, operation:string` | — | `0.0.1-dev` |
| `L-IO-0004` | `IO` | `Preview` | `Error` | 无法发现或读取测试输入 | Test input cannot be discovered or read | 测试输入文件操作失败 | test input file operation failed | `io_kind:string, operation:string, path:string` | — | `0.0.1-dev` |
| `L-IO-0005` | `IO` | `Preview` | `Error` | 无法发布工程构建产物 | Project build artifact cannot be published | 工程构建产物操作失败 | project build artifact operation failed | `io_kind:string, operation:string` | — | `0.0.1-dev` |
...
| `L-EFFECT-0001` | `EFFECT` | `Preview` | `Error` | Effect Row 约束无法满足 | Effect row constraint cannot be satisfied | Effect Row 约束冲突 | effect row constraint conflict | `conflict_set:string[], left_row:string, required_label?:string, right_row:string` | — | `0.0.1-dev` |
...
| `L-TASK-0001` | `TASK` | `Preview` | `Error` | Structured Task 静态结构无效 | Structured Task static structure is invalid | Structured Task 结构无效：{reason} | invalid Structured Task structure: {reason} | `actual?:string, expected?:string, reason:string, target?:string, task?:string` | — | `0.0.1-dev` |
| `L-TASK-0002` | `TASK` | `Preview` | `Error` | Task handle 所有权或观察规则无效 | Task handle ownership or observation rule is invalid | Task handle 使用无效：{reason} | invalid Task handle use: {reason} | `handle?:string, reason:string, scope?:string` | — | `0.0.1-dev` |
| `L-TASK-0003` | `TASK` | `Preview` | `Error` | Task suspension 活跃集不安全 | Task suspension live set is unsafe | Task suspension 活跃集不安全：{reason} | unsafe Task suspension live set: {reason} | `binding?:string, reason:string, suspension?:string` | — | `0.0.1-dev` |
| `L-TASK-0004` | `TASK` | `Preview` | `Error` | 已检查 Task 尚不能进入可执行阶段 | Checked Task cannot enter an executable stage yet | 已检查 Task 尚不能进入 {stage} 阶段 | checked Task cannot enter the `{stage}` stage yet | `definition:string, required_tasks:string, stage:string` | — | `0.0.1-dev` |
| `L-ACTOR-0001` | `ACTOR` | `Preview` | `Error` | Actor 声明违反已检查身份或状态隔离规则 | Actor declaration violates checked identity or state-isolation rules | Actor 声明无效：{reason} | invalid Actor declaration: {reason} | `actor?:string, actual?:string, expected?:string, reason:string` | — | `0.2.0-dev` |
| `L-ACTOR-0002` | `ACTOR` | `Preview` | `Error` | 已检查 Actor 尚不能进入可执行阶段 | Checked Actor cannot enter an executable stage yet | 已检查 Actor 尚不能进入 {stage} 阶段 | checked Actor cannot enter the `{stage}` stage yet | `definition:string, required_tasks:string, stage:string` | — | `0.2.0-dev` |
...
| `L-PROJECT-0001` | `PROJECT` | `Preview` | `Error` | 工程清单字节或大小无效 | Project manifest bytes or size are invalid | 工程清单的字节编码或大小无效 | project manifest byte encoding or size is invalid | `maximum_byte_len?:integer, reason:string, valid_up_to?:integer` | — | `0.0.1-dev` |
| `L-PROJECT-0002` | `PROJECT` | `Preview` | `Error` | 工程清单 TOML 结构无效 | Project manifest TOML structure is invalid | 工程清单的 TOML 结构无效 | project manifest TOML structure is invalid | `reason:string` | — | `0.0.1-dev` |
| `L-PROJECT-0003` | `PROJECT` | `Preview` | `Error` | 工程清单或语言版本不受支持 | Project manifest or language version is unsupported | 工程清单声明了不受支持的版本 | project manifest declares an unsupported version | `expected:string, field:string, found:string` | — | `0.0.1-dev` |
| `L-PROJECT-0004` | `PROJECT` | `Preview` | `Error` | 工程清单的包元数据无效 | Project manifest package metadata is invalid | 工程清单的包元数据无效 | project manifest package metadata is invalid | `field:string, reason:string, value?:string` | — | `0.0.1-dev` |
| `L-PROJECT-0005` | `PROJECT` | `Preview` | `Error` | 工程清单的源码布局无效 | Project manifest source layout is invalid | 工程清单的源码布局无效 | project manifest source layout is invalid | `field:string, reason:string, value?:string` | — | `0.0.1-dev` |
| `L-PROJECT-0006` | `PROJECT` | `Preview` | `Error` | 工程清单的导出模块无效 | Project manifest exported module is invalid | 工程清单包含无效的导出模块 | project manifest contains an invalid exported module | `module?:string, reason:string` | — | `0.0.1-dev` |
| `L-PROJECT-0007` | `PROJECT` | `Preview` | `Error` | 工程清单的本地依赖无效 | Project manifest local dependency is invalid | 工程清单包含无效的本地依赖 | project manifest contains an invalid local dependency | `dependency?:string, reason:string, value?:string` | — | `0.0.1-dev` |
| `L-PROJECT-0008` | `PROJECT` | `Preview` | `Error` | 工程源码根目录不存在、不可用或逃逸工程根 | Project source root is absent, unusable, or escapes the project root | 工程源码根目录无效 | project source root is invalid | `reason:string, root:string` | — | `0.0.1-dev` |
| `L-PROJECT-0009` | `PROJECT` | `Preview` | `Error` | 已发现源码路径无法确定性映射为 module | Discovered source path cannot map deterministically to a module | 工程源码路径无效 | project source path is invalid | `reason:string, source:string` | — | `0.0.1-dev` |
| `L-PROJECT-0010` | `PROJECT` | `Preview` | `Error` | module 声明缺失、重复、错位或与路径不匹配 | Module declaration is absent, duplicated, misplaced, or mismatched with its path | 工程源码的 module 声明无效 | project source has an invalid module declaration | `actual_module?:string, expected_module:string, reason:string` | — | `0.0.1-dev` |
| `L-PROJECT-0011` | `PROJECT` | `Preview` | `Error` | 多个源码文件映射到同一 module | Multiple source files map to the same module | 工程中存在重复 module | project contains a duplicate module | `first_source:string, module:string, second_source:string` | — | `0.0.1-dev` |
| `L-PROJECT-0012` | `PROJECT` | `Preview` | `Error` | entry、export 或本地 import 指向缺失 module | Entry, export, or local import names an absent module | 工程引用的 module 不存在 | project references an absent module | `importer?:string, module:string, role:string` | — | `0.0.1-dev` |
| `L-PROJECT-0013` | `PROJECT` | `Preview` | `Error` | import graph 包含 cycle 或命名空间冲突 | Import graph contains a cycle or namespace conflict | 工程 import graph 无效 | project import graph is invalid | `cycle?:string, module?:string, reason:string, target?:string` | — | `0.0.1-dev` |
| `L-PROJECT-0014` | `PROJECT` | `Preview` | `Error` | dependency graph 的路径、名称、身份或 cycle 无效 | Dependency graph has an invalid path, name, identity, or cycle | 工程依赖图无效 | project dependency graph is invalid | `actual?:string, cycle?:string, dependency?:string, expected?:string, first_content?:string, package:string, reason:string, second_content?:string` | — | `0.0.1-dev` |
| `L-PROJECT-0015` | `PROJECT` | `Preview` | `Error` | 工程分析超过实现资源上限 | Project analysis exceeds an implementation resource limit | 工程输入超过资源上限 | project input exceeds a resource limit | `actual:string, maximum:string, package:string, resource:string` | — | `0.0.1-dev` |
| `L-PROJECT-0016` | `PROJECT` | `Preview` | `Error` | 直接依赖包中不存在被导入的模块 | An imported module is absent from the direct dependency package | 依赖包中不存在被导入的模块 | the imported module does not exist in the dependency package | `dependency:string, importer:string, module:string, package:string` | — | `0.0.1-dev` |
| `L-PROJECT-0017` | `PROJECT` | `Preview` | `Error` | 依赖模块存在但未由其包公开导出 | A dependency module exists but is not publicly exported by its package | 被导入的依赖模块未公开导出 | the imported dependency module is not publicly exported | `dependency:string, importer:string, module:string, package:string` | — | `0.0.1-dev` |
| `L-PROJECT-0018` | `PROJECT` | `Preview` | `Error` | 工程锁文件的结构、身份或 canonical bytes 无效 | Project lockfile structure, identities, or canonical bytes are invalid | 工程锁文件无效 | project lockfile is invalid | `package?:string, reason:string` | — | `0.0.1-dev` |
| `L-PROJECT-0019` | `PROJECT` | `Preview` | `Error` | 工程锁文件缺失或与已解析依赖图不匹配 | Project lockfile is missing or does not match the resolved dependency graph | 工程锁文件与工程依赖图不匹配 | project lockfile does not match the project dependency graph | `reason:string` | — | `0.0.1-dev` |
| `L-LSP-0001` | `LSP` | `Preview` | `Warning` | LSP 诊断控制省略了重复或超限诊断 | LSP diagnostic control omitted duplicate or over-limit diagnostics | 已省略 {omitted} 条诊断 | {omitted} diagnostics omitted | `capped:integer, deduplicated:integer, maximum:integer, omitted:integer, scope:string` | — | `0.0.1-dev` |
| `L-LSP-0002` | `LSP` | `Preview` | `Error` | LSP 操作超过硬资源上限 | LSP operation exceeds a hard resource limit | 资源上限已超出 | resource limit exceeded | `actual:integer, maximum:integer, resource:string, scope:string` | — | `0.0.1-dev` |
| `L-QUERY-0001` | `QUERY` | `Preview` | `Error` | Semantic Query 标识符或作用域无效 | Semantic Query identifier or scope is invalid | Semantic Query 输入无效 | invalid Semantic Query input | `detail:string` | — | `0.0.1-dev` |
| `L-TEST-0001` | `TEST` | `Preview` | `Error` | 测试目标为空 | Test target selection is empty | 没有发现可运行的 `.ling` 测试文件 | no runnable `.ling` test files were found | `root:string` | — | `0.0.1-dev` |
| `L-TRANSACTION-0001` | `TRANSACTION` | `Preview` | `Error` | Semantic Transaction 输入或作用域无效 | Semantic Transaction input or scope is invalid | Semantic Transaction 输入无效 | invalid Semantic Transaction input | `detail:string` | — | `0.0.1-dev` |
| `L-TRANSACTION-0002` | `TRANSACTION` | `Preview` | `Error` | Semantic Transaction 基础 Program ID 已过期 | Semantic Transaction base Program ID is stale | Semantic Transaction 基础快照已过期 | Semantic Transaction base snapshot is stale | `expected:string, found:string` | — | `0.0.1-dev` |
| `L-TRANSACTION-0003` | `TRANSACTION` | `Preview` | `Error` | Semantic Transaction 违反声明的保持或授权约束 | Semantic Transaction violates a declared preserve or authorization constraint | Semantic Transaction 未保持要求的语义 | Semantic Transaction did not preserve required semantics | `constraint:string` | — | `0.0.1-dev` |
| `L-BYTECODE-0001` | `BYTECODE` | `Preview` | `Error` | 字节码封装、版本或记录 framing 无效 | Bytecode envelope, version, or record framing is invalid | 字节码封装无效：{reason} | invalid bytecode envelope: {reason} | `offset:integer, phase:string, reason:string, referenced_indices?:string[]` | — | `0.0.1-dev` |
| `L-BYTECODE-0002` | `BYTECODE` | `Preview` | `Error` | 字节码解码或验证超过资源上限 | Bytecode decoding or verification exceeds a resource limit | 字节码资源超过上限：{resource} | bytecode resource limit exceeded: {resource} | `actual:integer, maximum:integer, offset:integer, phase:string, reason:string, referenced_indices?:string[], resource:string` | — | `0.0.1-dev` |
| `L-BYTECODE-0003` | `BYTECODE` | `Preview` | `Error` | 字节码表或 canonical scalar 无效 | Bytecode table or canonical scalar is invalid | 字节码表或规范值无效：{reason} | invalid bytecode table or canonical value: {reason} | `offset:integer, phase:string, reason:string, referenced_indices?:string[]` | — | `0.0.1-dev` |
| `L-BYTECODE-0004` | `BYTECODE` | `Preview` | `Error` | 字节码指令、控制流、寄存器或类型状态无效 | Bytecode instruction, control flow, register, or type state is invalid | 字节码程序结构无效：{reason} | invalid bytecode program structure: {reason} | `offset:integer, phase:string, reason:string, referenced_indices?:string[]` | — | `0.0.1-dev` |
| `L-BYTECODE-0005` | `BYTECODE` | `Preview` | `Error` | 字节码 Effect、Capability 或入口不变量无效 | Bytecode Effect, Capability, or entry invariant is invalid | 字节码权限或入口无效：{reason} | invalid bytecode authority or entry: {reason} | `offset:integer, phase:string, reason:string, referenced_indices?:string[]` | — | `0.0.1-dev` |
| `L-BYTECODE-0006` | `BYTECODE` | `Preview` | `Error` | 字节码 source map 不完整或无效 | Bytecode source map is incomplete or invalid | 字节码源码映射无效：{reason} | invalid bytecode source map: {reason} | `offset:integer, phase:string, reason:string, referenced_indices?:string[]` | — | `0.0.1-dev` |
