...
| `PROTO-ACTOR-SEMANTIC-GRAPH-EXTENSION` | Public | JSON | `x-ling-actor/0.2` | `Experimental` | no | yes | 4 |
...
- Reader policy: Require exact x-ling-actor/0.2; validate Actor definition ownership, SendableLocal class, canonical contiguous message-schema order and digest, mailbox capacity 1..=65535, exact Reject policy, ling.checked-local-mailbox/1 canonical bytes, and original message/mailbox spans; return data only and never executable Core.
- Writer policy: Emit only from CheckedActorCore, its validated closed local message schema, and its immutable checked mailbox contract; preserve non-Actor ling.semantic/0.1 bytes and exclude paths, arena IDs, allocation, queue contents, runtime, scheduling, wire, remote, and host-representation facts.
...
- Migration tool: Manual source migration inserts `mailbox capacity 1 overflow Reject` or another accepted capacity before `state`; consumers require 0.2 exactly, and no 0.1 JSON adapter exists because 0.1 has no capacity.
- Authority: `DEC-0271`, `DEC-0272`
- Sources: [`crates/ling-concurrency/src/mailbox_contract.rs`](../../crates/ling-concurrency/src/mailbox_contract.rs), [`crates/ling-types/src/actor_message.rs`](../../crates/ling-types/src/actor_message.rs), [`crates/ling-effects/src/actor_core.rs`](../../crates/ling-effects/src/actor_core.rs), [`crates/ling-semantic/src/lib.rs`](../../crates/ling-semantic/src/lib.rs), [`docs/decisions/0272-checked-bounded-local-actor-mailbox.md`](../decisions/0272-checked-bounded-local-actor-mailbox.md), [`docs/decisions/0271-checked-actor-message-sendability-and-schema.md`](../decisions/0271-checked-actor-message-sendability-and-schema.md), [`docs/decisions/0270-checked-actor-identity-and-state-isolation.md`](../decisions/0270-checked-actor-identity-and-state-isolation.md), [`schemas/registry.toml`](../../schemas/registry.toml), [`schemas/semantic/0.1/schema.json`](../../schemas/semantic/0.1/schema.json), [`docs/governance/protocol-inventory.toml`](../governance/protocol-inventory.toml)
...
- Notes: The extension is checked-only and Experimental. It publishes pure Accepted/Full admission classification evidence but no send operation, Actor message execution, queue, suspension, loss policy, lifecycle, supervision, serialization, remote delivery, Resource, Managed, Borrow, or Capability transfer.
...
- Reader policy: Require the exact semantic, language, and Unicode versions; validate IDs, kinds, ownership, references, Prelude invariants, and ordering-independent structure. For x-ling-actor/0.2 also validate mailbox bounds, Reject policy, canonical contract bytes, and original spans; the returned graph is data only and cannot enter evaluation.
- Writer policy: Emit deterministic JSON from checked Typed Core with canonical ordering and no source paths, hash-map order, arena indices, allocation addresses, or Rust debug data in identity. Checked Task programs add the DEC-0264 x-ling-task/0.1 extension; checked Actor programs add the DEC-0272 x-ling-actor/0.2 extension with DEC-0271 SendableLocal/message-schema evidence plus the checked bounded local mailbox contract.
- Unknown-field policy: Accept x-* extension fields at checked object levels and reject unknown core fields.
- Migration tool: Actor consumers must require x-ling-actor/0.2 and source declarations must insert `mailbox capacity 1 overflow Reject` or another accepted capacity before `state`; there is no automatic 0.1 JSON adapter because 0.1 has no capacity. Other schema or identity changes require an explicit version upgrade, migration notes, and regenerated fixtures.
- Authority: `DEC-0012`, `RFC-0022`, `DEC-0264`, `DEC-0271`, `DEC-0272`
- Sources: [`crates/ling-concurrency/src/mailbox_contract.rs`](../../crates/ling-concurrency/src/mailbox_contract.rs), [`crates/ling-types/src/actor_message.rs`](../../crates/ling-types/src/actor_message.rs), [`crates/ling-effects/src/actor_core.rs`](../../crates/ling-effects/src/actor_core.rs), [`crates/ling-semantic/src/lib.rs`](../../crates/ling-semantic/src/lib.rs), [`docs/RFC-0022.md`](../RFC-0022.md), [`docs/decisions/0012-semantic-identity-and-canonical-bytes.md`](../decisions/0012-semantic-identity-and-canonical-bytes.md), [`docs/decisions/0264-structured-task-frontend-core.md`](../decisions/0264-structured-task-frontend-core.md), [`docs/decisions/0271-checked-actor-message-sendability-and-schema.md`](../decisions/0271-checked-actor-message-sendability-and-schema.md), [`docs/decisions/0272-checked-bounded-local-actor-mailbox.md`](../decisions/0272-checked-bounded-local-actor-mailbox.md), [`schemas/registry.toml`](../../schemas/registry.toml), [`schemas/semantic/0.1/schema.json`](../../schemas/semantic/0.1/schema.json), [`tools/xtask/src/schema.rs`](../../tools/xtask/src/schema.rs)
...
- Notes: GAP-SEMANTIC-PROTOCOL-LIFECYCLE-001 keeps Stable versus Experimental fields and cross-version migration open.; RFC-0022 defines the optional Experimental x-ling-trait-ide witness/member projection; it does not add a core field or an LSP wire method.; Accepted DEC-0264 adds the optional Experimental x-ling-task/0.1 checked-only projection without changing non-Task ling.semantic/0.1 bytes or authorizing execution.; Accepted DEC-0272 replaces the optional Experimental x-ling-actor/0.1 checked-only projection with x-ling-actor/0.2 bounded-mailbox evidence; non-Actor bytes remain unchanged and Actor execution, queue allocation, serialization, supervision, and remote delivery remain unavailable.
...
- Producer: VM-1202 ling-bytecode checked Typed Core lowerer and deterministic writer; VM-1203 canonical VerifiedProgramV1 re-encoder; VM-1205 closure/recursion lowerer and ling.bytecode/1.1 writer; VM-1206 aggregate/match lowerer and ling.bytecode/1.2 writer; VM-1208 checked Effect/Capability metadata boundary; VM-1209 table-driven Interpreter–VM differential harness; VM-1210 decoder/resource/cancellation evidence; EFF-2104 checked Handler and Cell/State bytecode 1.4 lowerer; EFF-2105 complete lexical Cell/State row lowerer
- Consumer: VM-1203 bounded independent decoder/verifier; VM-1204 verifier-gated ling-vm executor; VM-1205 closure/partial-application VM execution; VM-1206 aggregate/match VM execution; VM-1208 explicit host-capability preflight and Runtime Fault boundary; VM-1209 differential oracle over checked snapshots and verified bytecode; VM-1210 robustness and host-control test harness; EFF-2104 bytecode 1.4 Cell/Handler verifier and VM execution; EFF-2105 deterministic checked-source differential oracle
- Reader policy: The 1.4 reader accepts valid format (1, 0) through (1, 4) artifacts, dispatches on the exact version before decoding version-specific tables or instructions, validates hard and caller artifact bounds before allocation, rejects unknown executable content, and produces only untrusted decoded models. Every older reader rejects 1.4. The independent verifier is the sole constructor of VerifiedProgramV1, and ling-vm accepts only that verified state plus explicit limits and injected host Capabilities.
- Writer policy: The library-only writers accept checked-source lowering output or independently verified models. The 1.0 writer emits RFC-0014; 1.1 emits RFC-0015 closure/recursion; 1.2 emits RFC-0016 aggregates/match; 1.3 retains the historical immutable/irrefutable Handler slice; and 1.4 emits DEC-0262 typed Cell/State records for mutable lexical bindings and Handler captures. Every revision keeps canonical type, Effect, field, case, update, operation, and source-map order, zero reserved bytes, path-free metadata, and hard/caller limits. No CLI artifact contract is published.
...
- Authority: `RFC-0014`, `RFC-0015`, `RFC-0016`, `RFC-0018`, `RFC-0019`, `RFC-0020`, `RFC-0021`, `DEC-0261`, `DEC-0262`, `DEC-0263`
- Sources: [`docs/RFC-0014.md`](../RFC-0014.md), [`docs/RFC-0015.md`](../RFC-0015.md), [`docs/RFC-0016.md`](../RFC-0016.md), [`docs/RFC-0018.md`](../RFC-0018.md), [`docs/RFC-0019.md`](../RFC-0019.md), [`docs/RFC-0020.md`](../RFC-0020.md), [`docs/RFC-0021.md`](../RFC-0021.md), [`docs/decisions/0261-handler-runtime-and-bytecode.md`](../decisions/0261-handler-runtime-and-bytecode.md), [`docs/decisions/0262-handler-cell-state-bytecode.md`](../decisions/0262-handler-cell-state-bytecode.md), [`docs/decisions/0263-effect-property-generator-and-oracle.md`](../decisions/0263-effect-property-generator-and-oracle.md), [`docs/ROADMAP-1.0.md`](../ROADMAP-1.0.md), [`docs/governance/gap-register.toml`](../governance/gap-register.toml), [`crates/ling-bytecode/src/lib.rs`](../../crates/ling-bytecode/src/lib.rs), [`crates/ling-bytecode/src/lower/v1_1.rs`](../../crates/ling-bytecode/src/lower/v1_1.rs), [`crates/ling-bytecode/src/lower/v1_4.rs`](../../crates/ling-bytecode/src/lower/v1_4.rs), [`crates/ling-vm/src/lib.rs`](../../crates/ling-vm/src/lib.rs), [`crates/ling-vm/src/cancel.rs`](../../crates/ling-vm/src/cancel.rs), [`crates/ling-vm/src/execute.rs`](../../crates/ling-vm/src/execute.rs), [`crates/ling-vm/src/fault.rs`](../../crates/ling-vm/src/fault.rs), [`crates/ling-vm/src/value.rs`](../../crates/ling-vm/src/value.rs), [`crates/ling-vm/tests/support/effect_property.rs`](../../crates/ling-vm/tests/support/effect_property.rs), [`crates/ling-vm/tests/differential.rs`](../../crates/ling-vm/tests/differential.rs), [`fuzz/fuzz_targets/bytecode_bytes.rs`](../../fuzz/fuzz_targets/bytecode_bytes.rs), [`tests/bytecode/README.md`](../../tests/bytecode/README.md)
- Fixtures: [`tests/bytecode/v1/golden/hello.lbc.hex`](../../tests/bytecode/v1/golden/hello.lbc.hex), [`tests/bytecode/v1/golden/hello.dis`](../../tests/bytecode/v1/golden/hello.dis), [`tests/bytecode/v1/golden/cell-state-1.4.lbc.hex`](../../tests/bytecode/v1/golden/cell-state-1.4.lbc.hex), [`tests/bytecode/v1/golden/cell-state-1.4.dis`](../../tests/bytecode/v1/golden/cell-state-1.4.dis), [`tests/bytecode/v1/malformed-cases.tsv`](../../tests/bytecode/v1/malformed-cases.tsv), [`crates/ling-bytecode/tests/decode_verify.rs`](../../crates/ling-bytecode/tests/decode_verify.rs), [`crates/ling-bytecode/tests/lowering.rs`](../../crates/ling-bytecode/tests/lowering.rs), [`crates/ling-vm/tests/execution.rs`](../../crates/ling-vm/tests/execution.rs), [`crates/ling-vm/tests/differential.rs`](../../crates/ling-vm/tests/differential.rs)
- Notes: VM-1201 through VM-1204 implement the unverified data model, typed index/digest domains, fixed tags/opcodes/limits, checked-snapshot minimal lowering, deterministic writing, debug disassembly, bounded independent decoding, failure-atomic verification, canonical VerifiedProgramV1 re-encoding, verifier-gated execution, registered bilingual diagnostics, and valid/corrupt/fuzz/differential evidence. The protocol is Experimental, is not a DEC-0012 semantic canonical-byte format, and has no CLI artifact command, default backend, or N-1 compatibility claim.; Accepted RFC-0015 is implemented by VM-1205 as the backward-compatible ling.bytecode/1.1 closure/recursion extension. Both 1.0 and 1.1 remain Experimental; no Stable, CLI artifact, default-backend, or general N-1 release promise is implied.; Accepted RFC-0016 is implemented by VM-1206 as the backward-compatible ling.bytecode/1.2 aggregate and checked-match extension. All revisions remain Experimental; no Stable, CLI artifact, default-backend, or general N-1 release promise is implied.; Accepted RFC-0018 is implemented by VM-1208: Effect closure, explicit Capability preflight, source-mapped L-RUNTIME-0001 host Faults, and host-panic containment use the existing wire revisions. The protocol remains Experimental; VM-1209 differential corpus and VM-1210 fuzz/resource work remain separate.; Accepted RFC-0019 is implemented by VM-1209: the table-driven harness compares checked-interpreter and verifier-created VM logical events, Unit results, stable Fault projections, source spans, committed state, and deterministic ProgramId values.; Accepted RFC-0020 is implemented by VM-1210: the existing bytecode protocol gains bounded decoder/resource/cancellation evidence, while the experimental ling.vm.control/0.1 host API is inventoried separately and makes no wire or CLI promise.; Accepted RFC-0021 is implemented by the checked Trait member lowering slice: selected implementation DefinitionIds reuse existing direct-call instructions and do not add a wire revision or serialized dictionary table.; Accepted DEC-0261 reserved ling.bytecode/1.3 for verified first-order Handler execution. At that milestone the published current revision remained 1.2; the implemented 1.3 immutable/irrefutable slice now remains a historical Experimental revision under DEC-0262.; Accepted DEC-0262 is implemented by EFF-2104 and commit 1188b247: backward-reading ling.bytecode/1.4 has total binding/wildcard Handler inputs, exact Cell<T>/State<T> lowering, canonical writer/reader/disassembly/re-encoding, independent verification, non-cyclic heap-accounted VM Cells, and differential/Fault/cancellation/resource evidence. Revision 1.4 is the current Experimental library revision; 1.3 remains historical, older bytes remain unchanged, and no CLI artifact/default-backend or Stable promise is implied.; Accepted DEC-0263 is implemented by EFF-2105 and commit 3517ffcc: the deterministic checked-source oracle requires checked and verified State rows to agree, so bytecode 1.4 now selects every mutable lexical binding as an existing private Cell rather than only Handler-crossing captures. This corrects 1.4 canonical output for ordinary mutation without changing revisions 1.0 through 1.3, the source language, Program IDs, or any Stable/public replay promise.
