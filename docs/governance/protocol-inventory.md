...
- Writer policy: No LSP writer exists. A future writer must consume the RFC-0047 compiler-owned generation result and must obtain Accepted LSP-2403 transport authority before advertising a provider.
...
- Fixtures: —
- Notes: RFC-0047 authorizes only an in-process generation marker; no semanticTokensProvider, request method, client legend, result, public protocol marker, custom token type, or compatibility claim is implemented. Effect, Capability, resource, actor, node, kernel, Semantic ID, ownership, borrow, unsafe, and generated categories remain excluded.
