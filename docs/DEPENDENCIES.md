...
| `serde_json` | 1.0.151 | Deterministic Diagnostic/Semantic/REPL JSON, strict canonical `ling.lock/1` reading/writing, Audit string escaping, and conformance parsing | MIT OR Apache-2.0 | 1.71 | Yes, including validated UTF-8 fast paths | Current crates.io release inspected 2026-08-18; usage scope rechecked 2026-08-21 |
...
| `unicode-normalization` | 0.1.25 | Unicode 17 NFC normalization for identifiers and canonical bytecode logical source names | MIT OR Apache-2.0 | 1.36 | Yes, checked scalar construction and Hangul normalization | Reports `UNICODE_VERSION = (17, 0, 0)` and has a compile-time assertion; bytecode use adds no feature |
...
| `num-bigint` | 0.5.1 | Mathematical `Int` parsing, typed literals, canonical semantic bytes, interpreter arithmetic, canonical bytecode sign/magnitude lowering, and VM arbitrary-precision integer values/operations | MIT OR Apache-2.0 | 1.60 | Yes, validated UTF-8 construction, division internals, and target intrinsics | Current crates.io release inspected 2026-08-18; default `std` feature only; bytecode and VM reuse the locked workspace package |
...
| `sha2` | 0.11.0 | SHA-256 verification for pinned Unicode inputs, generated diagnostic compatibility fingerprints, and exact original-source digests in bytecode metadata | MIT OR Apache-2.0 | 1.85 | Yes, architecture-specific accelerated compression paths | Current crates.io release inspected 2026-08-18; bytecode reuses the locked workspace package and is not yet linked into the `ling` binary |
