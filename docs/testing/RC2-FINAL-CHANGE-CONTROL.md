...
Status: `BlockedSpec` (2026-08-23). This document records the proposed
...
The current RC3→RC1→RC0 bounded inventory chain passes, while all three
predecessor release gates remain `BlockedSpec`. That repository evidence does
not classify any change as a blocker or authorize RC2 entry.
...
| Affected protocol/artifact analysis | The 27-record protocol inventory and support matrix describe current boundaries; no candidate artifact/protocol manifest binds a change to release outputs. | BlockedSpec | Version/schema/diagnostic/Semantic ID/span/artifact impact map and compatibility decision. |
...
it also composes the current RC3 gate (which composes RC1 and RC0), but it does
not approve a blocker, create a candidate, or produce a release.
