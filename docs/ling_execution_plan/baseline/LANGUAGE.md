...
> 以下 `zero` 为内部占位命令；公开发布前必须改名。

```bash
zero init
zero repl
zero run main.zero
zero check main.zero
zero test
zero fmt
zero semantic main.zero
zero audit main.zero
zero explain Z-TYPE-0042
zero query --symbol 人物
zero patch transaction.json
zero build --profile explore
zero build --profile native
zero build --profile critical
```
