...
fn main() -> ExitCode {
    ...
    match args.as_slice() {
        ...
        [area, command] if area == "deprecation" && command == "verify" => {
            match deprecation_readiness::check_repository(&root) {
                Ok(summary) => {
                    println!(
                        "deprecation readiness OK: {} requirements ({} unavailable, {} guarded subset), {} released major versions",
                        summary.requirement_count,
                        summary.unavailable_count,
                        summary.guarded_subset_count,
                        summary.released_major_versions
                    );
                    ExitCode::SUCCESS
                }
                Err(errors) => {
        ...
        [area, command] if area == "deprecation" && command == "render" => {
            match deprecation_readiness::render_repository(&root) {
                Ok(output) => {
                    print!("{output}");
                    ExitCode::SUCCESS
                }
        ...
        [area, command] if area == "trait-performance" && command == "verify" => {
            match trait_performance_status::check_repository(&root) {
                Ok(summary) => {
                    println!(
                        "Trait performance inventory OK: {} surfaces ({} Internal, {} BlockedSpec), {} evidence files, {} task states",
                        summary.surface_count,
                        summary.internal_count,
        ...
        [area, command] if area == "project" && command == "verify" => {
            match project_status::check_repository(&root) {
                Ok(summary) => {
                    println!(
                        "project surface inventory OK: {} surfaces ({} Experimental, {} Internal, {} BlockedSpec), {} evidence files, {} task states",
                        summary.surface_count,
                        summary.experimental_count,
                        summary.internal_count,
                        summary.blocked_count,
